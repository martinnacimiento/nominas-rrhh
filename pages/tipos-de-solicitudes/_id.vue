...
div
  ...
  v-card.rounded-lg(:loading='$fetchState.pending', :elevation='12')
    v-card-title Editar tipo de solicitud
    v-card-text
...
<script>
export default {
  name: 'TiposSolicitudesId',
  async fetch() {
    const { id } = this.$route.params
    const { data } = await this.$axios.$get(`tiposSolicitud/${id}`)
    this.item = data
  },
  data: () => ({
    item: { area: '' },
  ...
  methods: {
    async save() {
      if (this.$refs.form.validate()) {
        try {
          this.loading = true
          await this.$axios.$put(`tiposSolicitud/${this.item.id}`, {
