  ...
  div
    ...
    v-card(class="rounded-lg" :elevation="12")
      v-data-table(
        ...
        template( v-slot:top)
          v-toolbar(flat)
            v-toolbar-title Tipos de solicitudes
            ...
            v-spacer
            v-btn(v-if="can('*')" color="primary" small :to="{name: 'tipos-de-solicitudes-nuevo'}")
              v-icon mdi-plus-circle-outline
        template(v-if="can('*') || can('*')" v-slot:item.actions="{ item }")
          v-icon(v-if="can('*')" small @click="$router.push({name: 'tipos-de-solicitudes-id', params: { id: item.id}})").mr-2 mdi-pencil
...
export default {
  async fetch() {
    const { data } = await this.$axios.$get('tiposSolicitud')
    this.items = data
  },
  data: () => ({
    items: [],
  ...
  methods: {
    ...
    async destroy(id) {
      try {
        await this.$axios.$delete(`tiposSolicitud/${id}`)
        this.get()
        this.resetDialog()
        this.snack('El tipo de solicitud ha sido eliminado.')
      } catch (error) {
