<template lang="pug">
  div
    ...
    v-card(raised)
      v-data-table.elevation-1(:headers='headers' :items='contracts' :search="search" sort-by='id')
        template( v-slot:top)
          v-toolbar(flat)
            ...
            v-spacer
            v-btn(color="primary" small :to="{name: 'contratos-nuevo'}")
        ...
        template( v-slot:item.actions="{ item }")
          v-icon(small @click="$router.push({name: 'contratos-id', params: { id: item.id}})").mr-2 mdi-pencil
